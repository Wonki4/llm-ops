...
function AnnouncementsPageInner() {
  ...
  const handleSave = () => {
    if (!title.trim() || !content.trim()) {
      toast.error(t("errorTitleContent"));
      return;
    }
    ...
    if (mode.kind === "create") {
      ...
      updateMutation.mutate(
        ...
        },
        {
          ...
          onError: (err) =>
            toast.error(err instanceof Error ? err.message : t("updateFail")),
        },
      );
    }
  };
  ...
  const handleSelectFromList = (id: string) => {
    if (mode.kind !== "view") {
      if (!confirm(t("confirmDiscardEdit"))) return;
    }
...
function ViewPane({
  ...
  return (
    <article className="p-6 space-y-4 overflow-y-auto">
      ...
      <div className="prose prose-sm dark:prose-invert max-w-none prose-headings:font-semibold prose-a:text-blue-600 dark:prose-a:text-blue-400 prose-pre:bg-gray-50 dark:prose-pre:bg-gray-800 prose-pre:border prose-pre:text-gray-900 dark:prose-pre:text-gray-100 prose-code:text-gray-900 dark:prose-code:text-gray-100">
      ...
      <div className="grid grid-cols-2 flex-1 min-h-0 divide-x">
        ...
        <div className="flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            {content.trim() ? (
              <div className="prose prose-sm dark:prose-invert max-w-none prose-pre:bg-gray-50 dark:prose-pre:bg-gray-800 prose-pre:border prose-pre:text-gray-900 dark:prose-pre:text-gray-100 prose-code:text-gray-900 dark:prose-code:text-gray-100">
                <ReactMarkdown
