...
- 키 기본 TPM/RPM: 이 팀에서 새로 발급되는 키에 적용되는 기본 스칼라 제한 (미설정 시 포털 전역 기본값)
- 모델별 한도: 모델별 TPM/RPM 제한. 드롭다운으로 모델 선택 (display_name 표시, 저장은 model_name), 모델마다 TPM/RPM 입력
  - **이 팀의 기존·신규 키 전부에 런타임 소급 적용** (LiteLLM_TeamTable.metadata 저장 → 키 무수정). [Rate Limit 정책](#rate-limit-tpmrpm-정책) 참고
- 멤버십 유효 기간: 가입 승인일로부터 자동 탈퇴 기간 (예: 90d)
...
### Rate Limit (TPM/RPM) 정책

키의 요청 제한은 **두 종류가 독립적으로(AND) 적용**된다. 두 체크를 모두 통과해야 요청이 허용된다.

| 종류 | 의미 | 저장 위치 | 적용 단위 |
|------|------|----------|----------|
| **스칼라 TPM/RPM** | 키 전체(모든 모델 합산) 상한 | `LiteLLM_VerificationToken.tpm_limit` / `rpm_limit` (컬럼) | 키마다 |
| **모델별 TPM/RPM** | 특정 모델별 상한 | metadata JSON (`model_tpm_limit` / `model_rpm_limit`) | 키마다 (팀값은 키별로 각각 적용, 공유 풀 아님) |

**모델별 한도 우선순위** (LiteLLM `get_key_model_tpm_limit`, 위에서부터 먼저 매칭):
1. 키 자신의 `metadata.model_tpm_limit` (키 오버라이드)
2. 키 `model_max_budget`
3. **팀** `metadata.model_tpm_limit` (팀 기본 → 그 팀의 모든 키에 상속)
4. deployment 기본값

- **팀 모델별 한도 = 소급**: 팀 metadata에 저장하면 그 팀의 기존·신규 키 전부에 런타임 적용(키 무수정). 변경은 캐시 TTL(~60초) 후 반영.
- **키 오버라이드 = 통째 교체**: 키에 모델별 값이 하나라도 있으면 팀값을 병합하지 않고 무시한다. 그래서 키 편집 UI는 팀값을 prefill한 뒤 전체 세트를 저장한다(관리자 → 사용자 관리 → 키 수정). 행을 모두 비우면 다시 팀값을 상속한다.
- **저장값은 항상 model_name**: 드롭다운은 display_name을 보여주지만 저장/매칭은 기술 model_name으로 한다.

> **스칼라 vs 모델별 충돌 주의**: 둘은 AND로 걸리므로 **모델별 한도를 스칼라보다 높게 잡아도 스칼라에서 먼저 막힌다**. 예) 키 스칼라 100,000 TPM + 팀 모델별 gpt-4o 200,000 → 실효 100,000. 현재 기존 키 대부분은 스칼라 NULL(전체 무제한)이라 충돌이 없으며, 스칼라 기본값(포털/팀 설정)은 신규 키 생성 시점에만 박힌다(소급 안 됨). 모델별을 주 제어로 쓰려면 스칼라 기본값을 충분히 높게 두거나 비워야 한다.
...
| PUT | `/api/teams/{id}/settings` | 팀 설정 (멤버 기본 예산, 키 기본 TPM/RPM, 모델별 한도) |

### 키
| Method | Path | 설명 |
|--------|------|------|
| POST | `/api/keys` | 키 생성 (sk-JWT) |
| GET | `/api/keys` | 내 키 목록 (모델별 유효 한도 + 팀 상속 여부 포함) |
| GET | `/api/keys/{hash}/reveal` | 키 복사 |
| DELETE | `/api/keys/{hash}` | 키 삭제 |
| PATCH | `/api/admin/users/{uid}/keys/{token}/limits` | (관리자) 키 TPM/RPM + 모델별 한도 오버라이드 |
