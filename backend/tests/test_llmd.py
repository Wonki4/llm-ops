...
async def test_create_stack_with_direct_upserts_service_and_ingress(client_for_user, super_user, mock_db):
    ...
    ing_svc = by_kind["Ingress"]["spec"]["rules"][0]["http"]["paths"][0]["backend"]["service"]
    assert ing_svc["name"] == "llmd-demo-direct"


async def test_create_stack_direct_without_labels_400(client_for_user, super_user, mock_db):
    mock_db.execute = AsyncMock(return_value=_none_result())
    fake_target = MagicMock()
    fake_target.create_or_patch = AsyncMock()
    with patch(
...
async def test_update_toggle_off_deletes_direct(client_for_user, super_user, mock_db):
    stack = _stack(name="demo", namespace="team-a", argo_app_name="llmd-demo", direct_route_enabled=True)
    mock_db.execute = AsyncMock(return_value=_result_with(stack))
    fake_k8s = MagicMock()
    fake_k8s.apply_application = AsyncMock()
    fake_k8s.get_application = AsyncMock(return_value=None)
    ...
    fake_target.delete.assert_awaited_once_with(
        "team-a", {"service": "llmd-demo-direct", "ingress": "llmd-demo-direct-ingress"}
    )


async def test_delete_stack_enabled_removes_direct(client_for_user, super_user, mock_db):
    stack = _stack(name="demo", namespace="team-a", argo_app_name="llmd-demo", direct_route_enabled=True)
    mock_db.execute = AsyncMock(return_value=_result_with(stack))
    fake_k8s = MagicMock()
    fake_k8s.delete_application = AsyncMock()
...
def test_serialize_reports_direct_fields():
    base = _serialize(_stack(argo_app_name="llmd-demo"), {"sync_status": "Synced"})
    assert base["direct_route_enabled"] is False
    assert base["direct_ingress_host"] == "llmd-demo-direct.llm-d.local"
    assert base["direct_service"] == "llmd-demo-direct"
    assert base["direct_overrides"] == {"ingress_host": None}
    over = _serialize(
        _stack(argo_app_name="llmd-demo", direct_route_enabled=True, direct_ingress_host="sticky.corp"),
        {"sync_status": "Synced"},
    )
    assert over["direct_route_enabled"] is True
    assert over["direct_ingress_host"] == "sticky.corp"
    assert over["direct_overrides"] == {"ingress_host": "sticky.corp"}
